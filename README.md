# Yol-Haritası
# CSEH Academy tarafından siber güvenlikle ilgili sorularınıza cevap olması amacıyla hazırlanmıştır.


- 💻 Bilgisayar Özellikleri (Minimum Öneri)

Minimum i5 veya Ryzen 5 işlemci

Minimum 8 GB RAM (en kritik bileşen)

Aynı anda 2 veya daha fazla sanal makine çalıştırabilmelisiniz

Örn: saldırgan makine + kurban makine

Mutlaka SSD

240 GB SSD, 1 TB HDD’den daha iyidir

------------------------------

- ❓ “XX yaşındayım, geç kaldım mı?”

Hayır. Teknoloji alanında geç kalmak diye bir şey yok.

Önemli olan:

doğru yöntemle çalışmak

araştırmayı bilmek

öğrenmeyi öğrenmek

İlerleyen yaş, motivasyon ve disiplinle avantaj bile olabilir.

------------------------------

- 🎓 Öğrenciyim, ne gibi haklarım var?

Ortaöğretim ve üzerindeyseniz:

👉 GitHub Education Pack ile birçok ürüne ücretsiz erişebilirsiniz
(domain + hosting dahil)

🔗 https://education.github.com/pack

------------------------------

- 🗺️ Yol haritası nereden bulacağım?

Hazır roadmap siteleri:

🔗 https://roadmap.sh

ilgili alanı seç

maddeler halinde ilerle

- 👨‍💻 Programlama geçmişim var, hangi alana yönelmeliyim?

En hızlı geri dönüş alınan alanlar:

Application Pentesting

Security Researcher

Başlamak için:

OWASP dökümantasyon

Bug bounty platformları:

HackerOne

Bugcrowd

Eğitim bölümleri de var – doğrudan para kazanmaya başlayabilirsin.

------------------------------

- 🔐 Hack saldırılarından nasıl korunurum?

güçlü şifre kullan

büyük/küçük harf + sayı + özel karakter

tercihen 12+ karakter

şifreleri tekrar kullanma

password manager kullan

2FA mutlaka aktif et

Şifren sızmış mı kontrol et:

🔗 https://haveibeenpwned.com/

------------------------------

- 🐧 Linux’u nereden öğreneceğim?

Başlangıç için:

https://cmdchallenge.com

Türkçe sıfırdan kaynak:

https://github.com/LuNiZz/siber-guvenlik-sss/blob/master/Linux-ve-Pentesting-Baslangic.md

- 🌐 Web uygulama güvenliği için kaynak

Türkçe:

MDISEC (üst düzey içerik)
🔗 https://youtu.be/WtHnT73NaaQ

Yabancı:

The Cyber Mentor playlist
🔗 https://www.youtube.com/watch?v=X4eRbHgRawI

Lab ortamları:

bWAPP → http://www.itsecgames.com

PortSwigger Academy → https://portswigger.net/

------------------------------

- 🎓 Hangi programlama dillerine ağırlık vermeliyim?

Önerilen teknolojiler:

Python

JavaScript

C / C++

Bash scripting

SQL temelleri

Kali Linux sanal makine kullanımı

Temel ağ bilgisi (CCNA seviyesi)

------------------------------

- 🐍 Python için kaynak önerileri

Türkçe:

https://python-istihza.yazbel.com/

Sadi Evren Şeker playlist
🔗 https://youtu.be/AaOv4BjN2UY

Yabancı:

🔗 https://youtu.be/rfscVS0vtbw

- 🇬🇧 İngilizcem yok, ne yapacağım?

Kısa cevap: öğreneceksin 🙂

Öneriler:

Duolingo vb. uygulamalar

altyazılı dizi–film

Voscreen → https://www.voscreen.com/

Siber güvenlikte teknik İngilizce şart.

------------------------------

- 🌐 Networking öğrenmek istiyorum

Hedef:

CCNA düzeyi

Öneri playlist:

🔗 https://www.youtube.com/playlist?list=PLDQaRcbiSnqF5U8ffMgZzS7fq1rHUI3Q8

- 🧪 Pentesting çalışacağım ama kimseye zarar vermek istemiyorum

Yasal ve güvenli lab ortamları:

https://tryhackme.com
 (başlangıç–orta)

https://www.hackthebox.com
 (orta–ileri)

Türkçe:

https://priviahub.com

------------------------------

- 🌍 VPN’i nasıl kurarım?

Kişisel VPN ve gizlilik konuları için Udemy kursu:

🔗 https://www.udemy.com/course/uygulamali-etik-hacker-olma-kursu/?referralCode=A2C91BDCEC549705CC1E

------------------------------

- 🎖️ Hangi sertifikaları almalıyım?

Popüler ve etkili:

OSCP

CISSP (daha kurumsal / bilgi güvenliği odaklı)

Not:
Bunlar pahalıdır. Çoğu zaman şirket sponsor olur.
Başlangıç için zorunlu değiller.

------------------------------

- 🤖 Yapay Zekâ ve Siber Güvenlik – Neden Önemli?

Yapay zekâ (AI) günümüzde:

saldırıların otomasyonunda

zafiyet tespitinde

zararlı yazılım analizinde

SOC ve SIEM sistemlerinde

tehdit istihbaratında

aktif şekilde kullanılmaktadır.

Aynı zamanda saldırganlar da AI kullanıyor:

phishing e-mailleri otomatik üretme

parola deneme saldırılarını optimize etme

kötü amaçlı kod oluşturma

deepfake ile sosyal mühendislik

Bu yüzden siber güvenlik + yapay zekâ birleşimi, geleceğin en kritik alanlarından biridir.

------------------------------

- 🧠 Hangi Yapay Zekâ Alanları Siber Güvenlikle Kesişir?

Machine Learning for Security

Adversarial Machine Learning

Malware Classification

Network Anomaly Detection

Fraud Detection

Threat Intelligence Automation

LLM Security & Prompt Injection Analizi

AI Red Teaming

------------------------------

- 🛡️ AI destekli savunma neler yapabilir?

loglardan saldırı tespit etme

anomali tabanlı IDS/IPS sistemleri geliştirme

phishing tespiti

zararlı yazılım ailelerini sınıflandırma

otomatik raporlama ve olay analizi

otomatik yara izi (IOC) çıkarımı

------------------------------

- ⚠️ AI destekli saldırılar

Günümüzde saldırganların AI’yı kullanabildiği alanlar:

otomatik OSINT toplama

sosyal mühendislik mesajları üretme

phishing site ve e-mail üretimi

exploit ve malware varyantları oluşturma

LLM zafiyetlerini kullanma (prompt injection vs.)

Not: Tüm çalışmalar hukuka uygun lab ortamlarında ve eğitim amaçlı yapılmalıdır.

- 🛠️ AI + Siber Güvenlik için hangi teknolojiler öğrenilmeli?

Önerilen teknoloji seti:

Python

Numpy / Pandas

Scikit-Learn

PyTorch veya TensorFlow

Jupyter Notebook

Regex

Wireshark + PCAP analizi

YARA kuralları

ELK Stack (Elastic – Logstash – Kibana)

------------------------------

- ✅ Son Tavsiye

açık kaynak projelere katkı ver

Pull Request gönder

blog / repo / notlarını paylaş

topluluklarda aktif ol

“Siberci” olmanın yolu sürekli üretmekten geçer.

✍️ Kaynakça ve Katkı

Orijinal içerik: LuNiZz

Düzenleme ve eklemeler: CSEH Academy
